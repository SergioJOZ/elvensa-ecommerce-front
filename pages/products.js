  ...
  return (
    <>
      ...
      <>
        <Center>
          <WhiteBox>
            <h1 className="text-3xl text-center">Cátalogo</h1>
            <StyledDivForm>
              ...
              <form onSubmit={busquedaFuncion}>
                <StyledSelect
                  label="Busca por tipo de producto"
                  value={busquedaTipo}
            ...
            <div className="grid grid-cols-3 gap-20 pt-10">
              {filteredProducts.length > 0 &&
                filteredProducts.map((product) => (
                  <Card key={product._id} className="w-44">
          ...
          {product.description}
                    ...
                    <CardFooter className="pt-0">
                      <Typography color="blue-gray" className="font-medium">
                        {product.price}$ /{" "}
                        {Math.round(
                          (product.price * bsVar[0].bsVariable * 100) / 100
                        ).toFixed(2)}{" "}
                        Bs
                      </Typography>
...
//TRAE LA INFORMACIÓN DE LOS PRODUCTOS DESDE LA BASE DE DATOS
